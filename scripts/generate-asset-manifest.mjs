...
async function generateManifest(collection) {
  const assetsDir = path.join(imagesRoot, collection);
    ...
    Object.entries(manifest)
      ...
      .map(([id, filenames]) => [
        ...
        filenames
          ...
          .map((filename) => `/images/${collection}/${filename}`)
      ])
  );
...
Promise.all(collections.map((collection) => generateManifest(collection))).catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
